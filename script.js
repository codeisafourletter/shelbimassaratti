...
photoStyles.textContent=`
  ...
  @media(max-width:900px){.work-photo-proof__head{display:grid}.work-photo-grid{grid-template-columns:1fr 1fr;grid-auto-rows:250px}.work-photo:first-child{grid-row:span 1;grid-column:span 2}.work-photo:nth-child(4){grid-column:span 1}}
  @media(max-width:560px){.work-photo-grid{grid-template-columns:1fr;grid-auto-rows:280px}.work-photo:first-child,.work-photo:nth-child(4){grid-column:auto}.work-photo-proof__head h3{font-size:2.65rem}}
`;
...
if(recordGrid){
  ...
  proof.innerHTML=`
    ...
    <div class="work-photo-grid">
      ${workPhotos.map(item=>`<figure class="work-photo"><img src="${item.src}" alt="${item.alt}" loading="lazy" decoding="async"><figcaption><span>${item.eyebrow}</span><strong>${item.title}</strong></figcaption></figure>`).join('')}
    </div>
